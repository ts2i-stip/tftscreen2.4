...
void loop()
{
  bool boutonMarche1Presse = digitalRead(boutonMarche1Pin) == LOW;
  bool boutonMarche2Presse = digitalRead(boutonMarche2Pin) == LOW;
  if (is2ButtonsPressed())
  {
    // Les deux boutons "Marche 1" et "Marche 2" sont pressés simultanément
    Serial.println("Départ cycle, sortie vérin épaisseur");
    digitalWrite(CYL_CNT_WIDTH, HIGH); // Active le vérin
    while (digitalRead(SEN_CYL_CNT_WIDTH) == LOW)
    {
      // Attendez
    }
    delay(3000);                          // Attend 2 secondes
    Serial.println("---->fin sortie vérin épaisseur");
    int position1 = monEncodeur1.read(); // Lire la position de l'encodeur
    Serial.print("Position de l'encodeur1 : ");
    Serial.println(position1);
    Serial.print("Conversion num 2 chaine : ");
    char mystr[7];
    sprintf(mystr, "%d", position1);
    ...
    if (position1 >= valeurMin1 && position1 <= valeurMax1)
    {
      ...
      while (digitalRead(SEN_CYL_CNT_HOLE) == LOW)
        ;
      delay(3000);                          // Attend 2 secondes
      Serial.println("----> Fin sortie vérin ouverture");
      long position2 = monEncodeur2.read(); // Lire la position de l'encodeur
      ...
      if (position2 >= valeurMin2 && position2 <= valeurMax2)
      {
        ...
        while (digitalRead(SEN_CYL_CNT_MARK) == LOW)
        {
          // Attendez
        }
